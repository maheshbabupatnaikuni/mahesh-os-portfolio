...
export default function ProjectPage() {
  ...
  const projectImage = `${import.meta.env.BASE_URL}${project.image.replace(/^\//, '')}`

  return <main id="main-content" className="case-study route-main" tabIndex={-1} style={{ '--project-accent': project.accent } as React.CSSProperties}>
    <header className="case-hero">
      ...
      <img src={projectImage} alt={`${project.name} visual overview`} />
    </header>
    <div className="case-body">
      ...
      <div className="case-content">
        ...
        <section className="screenshots"><h2>Concept interface preview</h2><p>This graphic is a portfolio concept visual, not a confidential application screenshot.</p><div><img src={projectImage} alt={`${project.name} concept interface preview`} loading="lazy"/></div></section>
        ...
        {(project.experiences.liveDemo !== 'unavailable' && project.liveUrl || project.experiences.sourceCode && project.githubUrl) && <div className="case-actions">{project.experiences.liveDemo !== 'unavailable' && project.liveUrl && <a href={project.liveUrl} data-analytics-event="project_live_click" data-analytics-project-id={project.slug} data-analytics-project-name={project.name} data-analytics-location="case_study">{(project.actionLabel || 'OPEN LIVE SITE').toUpperCase()} <ArrowUpRight/></a>}{project.experiences.sourceCode && project.githubUrl && <a href={project.githubUrl} data-analytics-event="project_repository_click" data-analytics-project-id={project.slug} data-analytics-project-name={project.name} data-analytics-location="case_study">GITHUB <ArrowUpRight/></a>}</div>}
      </div>
    </div>
  </main>
}
