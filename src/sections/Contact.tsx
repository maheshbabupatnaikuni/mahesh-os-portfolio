...
export function Contact() {
  return <section id="contact" className="section contact story-section">
    ...
    <div className="contact-layout"><div className="contact-details"><a href={`mailto:${profile.email}`}><Mail/><span>Email<small>{profile.email}</small></span><ArrowUpRight/></a><a href={`tel:${profile.phone.replace(/\s/g, '')}`}><Phone/><span>Phone<small>{profile.phone}</small></span><ArrowUpRight/></a><a href={profile.linkedin} target="_blank" rel="noreferrer"><Linkedin/><span>LinkedIn<small>Connect professionally</small></span><ArrowUpRight/></a><div><MapPin/><span>Current location<small>{profile.location}</small></span></div><div><MapPin/><span>Home background<small>{profile.hometown}</small></span></div></div><ContactForm/><aside className="portfolio-qr"><img src={`${import.meta.env.BASE_URL}share/mahesh-portfolio-qr.svg`} alt="QR code opening P. Mahesh Babu's public portfolio" loading="lazy"/><div><p className="eyebrow">SCAN / SHARE / SAVE</p><h3>Take this portfolio with you.</h3><p>Scan the code to open the public portfolio. Use the high-resolution PNG in a resume, LinkedIn post, Naukri profile or printed document.</p><div className="qr-actions"><a href={`${import.meta.env.BASE_URL}share/mahesh-portfolio-qr.png`} download="P-Mahesh-Babu-Portfolio-QR.png">DOWNLOAD QR</a><a href={profile.portfolioUrl} target="_blank" rel="noreferrer">OPEN PUBLIC SITE <ArrowUpRight size={15}/></a></div><small>{profile.portfolioUrl}</small></div></aside></div>
  </section>
}
